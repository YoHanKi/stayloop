 ...
 * `ReservationRepositoryImpl` 이 위임한다 (3-class 분리, `.github/instructions/repository.instructions.md`).
 *
 * 본 인터페이스는 `JpaRepository` *만* 상속하며 별도 메서드를 갖지 않는다 — overlap 조회 등 도메인성 쿼리는
 * `ReservationRepositoryImpl` 안의 **QueryDSL** 로 표현 (문자열 JPQL `@Query` 미사용).
 */
